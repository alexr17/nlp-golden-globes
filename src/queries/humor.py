                    ...
                    for i in range(len(tokens1)):
                        if tokens1[i] in funny_list:
                            if funny_words != -1:
                                ...
                                funny_words = 1
                    if funny_words != -1:
                       # if any(x in tweet_without_quote for x in funny_list):
                        tokens = bigrams(nltk.word_tokenize(tweet_without_quote), funniest_jokes_kw, gg_sw + funny_list)
                        for tkn in tokens:
                            tkn = tkn.lower()
                            if tkn in names:
                                if tkn not in funny_jokes_dict:
                                    funny_jokes_dict[tkn] = [obj['text'][start_quote_index:end_quote_index+1], 1, obj['text']]
                                else:
                                    if funny_jokes_dict[tkn][0] == obj['text'][start_quote_index:end_quote_index+1]:
                                        funny_jokes_dict[tkn][1] += 1
                                        funny_jokes_dict[tkn][2] = obj['text']
    ...
    for i in range(25):
        if i < len(jokes_list):
            if jokes_list[i][1][1]>0:
                print ('Celebrity: ' + jokes_list[i][0] +'\n'
                       +'Joke: '+ jokes_list[i][1][0] + '\n'
                       +'Full tweet: '+ jokes_list[i][1][2]+ '\n'
